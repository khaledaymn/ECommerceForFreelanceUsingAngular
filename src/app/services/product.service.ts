...
import { CreateProduct, Product, ProductParams, UpdateProduct } from "../interfaces/product.interface"
...
export class ProductService {
  ...
  private readonly allowedImageExtensions = ['.png', '.jpg', '.jpeg', '.webp', '.svg'];
  ...
  private readonly errorMessages: Record<string, string> = {
    ...
    imageType: 'Invalid image file type. Allowed types: .png, .jpg, .jpeg, .webp, .svg.',
    videoSize: 'Video file size exceeds 10MB.',
    videoType: 'Invalid video file type. Allowed types: .mp4, .webm, .mov, .mkv.'
  };
  ...
  createProduct(product: CreateProduct): Observable<Result> {
    ...
    if (product.mainImage && !this.validateImageFile(product.mainImage)) {
      return throwError(() => new ApiError(400, 'validation', this.errorMessages['imageType']));
    }
    if (product.additionalImages && !this.validateMediaFiles(product.additionalImages)) {
      return throwError(() => new ApiError(400, 'validation', this.errorMessages['videoType']));
    }
    ...
    if (product.additionalAttributes) {
      formData.append('additionalAttributes', JSON.stringify(product.additionalAttributes));
    }
    ...
    if (product.additionalImages) {
      product.additionalImages.forEach((file, index) => formData.append(`additionalImages[${index}]`, file));
    }

    return this.http.post<Result>(`${this.baseUrl}/Create`, formData).pipe(
      retry({ count: this.maxRetries, delay: 1000 }),
      catchError((error) => this.handleError(error, 'createProduct', { name: product.name }))
    );
  }

  getAllProducts(params: ProductParams = { pageIndex: 1, pageSize: 10 }): Observable<PaginatedResponse<Product>> {
  ...
  readProductById(id: number): Observable<Product> {
    if (id < 1) {
      return throwError(() => new ApiError(400, 'validation', 'Product ID must be a positive integer.'));
    }
  ...
  updateProduct(product: UpdateProduct): Observable<Result> {
    ...
    return this.http.put<Result>(`${this.baseUrl}/Update`, formData).pipe(
      retry({ count: this.maxRetries, delay: 1000 }),
      catchError((error) => this.handleError(error, 'updateProduct', { id: product.id }))
    );
  }
