...
export class CategoryService {
  ...
  getAllCategories(
    ...
    return this.http
      .get<PaginatedResponse<Category>>(`${this.baseUrl}/GetAllCategories`, {
        params: httpParams,
      })
      .pipe(
  ...
  getCategoryById(id: number): Observable<Category> {
    return this.http
      .get<Category>(`${this.baseUrl}/GetCategoryById/${id}`)
      .pipe(
        retry({ count: this.maxRetries, delay: 1000 }),
        catchError((error) =>
  ...
  createCategory(category: AddCategory): Observable<Result> {
    ...
    if (category.image && !this.validateImage(category.image)) {
      return throwError(
        () => new ApiError(400, 'validation', this.errorMessages['fileType'])
      );
    }
    ...
    return this.http.post<Result>(`${this.baseUrl}/AddCategory`, formData).pipe(
      ...
      catchError((error) =>
        this.handleError(error, 'createCategory', { name: category.name })
      )
    );
  }
  ...
  updateCategory(category: UpdateCategory): Observable<Result> {
    ...
    return this.http
      .put<Result>(`${this.baseUrl}/UpdateCategory`, formData)
      .pipe(
        retry({ count: this.maxRetries, delay: 1000 }),
        catchError((error) =>
          this.handleError(error, 'updateCategory', { id: category.id })
        )
      );
  }
  ...
  deleteCategory(id: number): Observable<Result> {
    return this.http
      .delete<Result>(`${this.baseUrl}/DeleteCategory/${id}`)
      .pipe(
        retry({ count: this.maxRetries, delay: 1000 }),
