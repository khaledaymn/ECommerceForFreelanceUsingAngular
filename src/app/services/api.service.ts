...
import { ApiResponse, ApiListResponse, ApiErrorResponse } from "../interfaces/api-response.interface"
...
export class ApiService {
   ...
   */
  getList<T>(endpoint: string, params?: any): Observable<{ data: T[]; meta: any }> {
    const options = { params: this.buildParams(params) }

    return this.http.get<ApiListResponse<T>>(`${this.apiUrl}/${endpoint}`, options).pipe(
      map((response) => ({
        data: response.data,
        meta: response.meta,
      })),
