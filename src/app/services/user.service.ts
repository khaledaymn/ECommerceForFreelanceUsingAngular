...
export class UserService {
  ...
  private cache = new Map<string, { data: UsersResponse | User; timestamp: number }>();
  ...
  updateUser(id: string, user: UserDTO): Observable<{ message: string }> {
    return this.http.put<{ message: string }>(`${this.apiUrl}/Users/UpdateUser/${id}`, user).pipe(
      tap(() => {
  ...
  toggleUserBlock(id: string, isBlock: boolean): Observable<{ message: string }> {
