...
import { User, UsersResponse, UsersFilter, UserDTO } from '../../../interfaces/user.interface';
...
export class UsersComponent implements OnInit {
  columns: TableColumn[] = [
    ...
    { key: 'isDeleted', title: 'الحالة', type: 'boolean', sortable: true, width: '10%' },
  ];
// type boolean
  actions: TableAction[] = [
  ...
  confirmDialog: { isOpen: boolean; userId: string | null; title: string; description: string; action: 'delete' | 'toggleBlock' | null } = {
    isOpen: false,
    userId: null,
    title: '',
    description: '',
  ...
  openToggleBlockDialog(user: User): void {
    ...
    this.confirmDialog = {
      ...
      description: `هل أنت متأكد من ${isBlocked ? 'إلغاء حظر' : 'حظر'} المستخدم ${user.email}؟`,
      action: 'toggleBlock',
    };
  }
  ...
  getFilteredIsDeletedOptions() {
    ...
    return this.isDeletedOptions.filter(option =>
      option.label.toLowerCase().includes(this.isDeletedSearchTerm.toLowerCase())
    );
  }
  ...
  hasActiveFilters(): boolean {
    return this.isDeletedFilter !== undefined || this.isBlockedFilter !== undefined;
  }
